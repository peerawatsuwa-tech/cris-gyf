...
const missions: MissionDefinition[] = [
  ...
  },
  {
    ...
    icon: Radar,
    equipment: ["radar", "communication", "eoir", "weapon"],
    crewWeight: 0.25,
  },
  {
    id: "M4",
...
export default function MissionCapabilityPanel() {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
      <header className="flex flex-col gap-3 border-b border-slate-800 px-6 py-5 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-sm font-semibold text-sky-300">
            ขีดความสามารถตามภารกิจ M1–M8
          </p>
          <h2 className="mt-1 text-xl font-bold text-white">
            ระบบแปลงความพร้อมของเรือเป็นผลกระทบต่อภารกิจ
          </h2>
        </div>

        <span className="rounded-full border border-amber-500/30 bg-amber-950/20 px-3 py-1.5 text-xs font-bold text-amber-300">
          ภารกิจที่ต้องเฝ้าระวัง {atRisk} ภารกิจ
        </span>
      </header>
      ...
      <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4">
        {missionResults.map((mission) => {
          ...
          return (
            <article
              key={mission.id}
              className={`rounded-xl border bg-slate-950/55 p-4 ${meta.borderClass}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 items-center gap-3">
                  <div className="rounded-lg bg-sky-950 p-2.5 text-sky-300">
                ...
                <span className={`text-lg font-black ${meta.textClass}`}>
                  {mission.status}
                </span>
              </div>
              ...
              <div className="mt-4 flex items-end justify-between gap-3">
                <div>
                  <span className={`text-3xl font-black ${meta.textClass}`}>
                    {mission.score}
                  </span>
      ...
      <footer className="border-t border-slate-800 px-6 py-3 text-xs leading-5 text-slate-500">
        ผลประเมินเป็นข้อมูลสาธิตจากกำลังพล ระบบเรดาร์ การสื่อสาร
        ระบบอาวุธ การเดินเรือ EO/IR และ RHIB ของเรือทั้งกองเรือ
      </footer>
    </section>
  );
}
